...
class Unet(object):
    ...
    def __pool_layer(self, input, filters, block_num, drop_prob=.3, activation='linear', padding='same',
        ...
        input_1 = Conv2D(filters, 1, activation=activation, padding=padding, kernel_initializer=init,
                       kernel_regularizer=reg, name=prefix + 'a2')(input)
        conv1 = Add()([conv1, input_1])  #added residual             
    ...
    def __unpool_block(self, pooled, prepooled, block_num, drop_prob=.2, activation='linear', padding='same',
        ...
        pooled_1 = Conv2DTranspose(filters, kernel_size=(1, 1), strides=2, padding=padding, activation=activation,
                                kernel_initializer=init, kernel_regularizer=reg, name=prefix + 'upsampledss32')(pooled)

        conv1 = Add()([conv1, pooled_1])  #added residual            
        conv1 = LeakyReLU()(conv1)
        conv1 = BatchNormalization()(conv1)
