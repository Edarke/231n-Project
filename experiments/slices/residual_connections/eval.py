...
def crf(inputs_all, predictions_all):
    for i in range(inputs_all.shape[0]):
        ...
        d.addPairwiseGaussian(sxy=228, compat=4)

        ##### d = dcrf.DenseCRF(inputs.shape[0]*inputs.shape[1], 4)
        ##### d.addPairwiseEnergy(inputs.shape[0],inputs.shape[1], 4,inputs.shape[0]*inputs.shape[1])
