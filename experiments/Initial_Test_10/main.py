...
class Net2D(object):
    def __init__(self, config):
        self.config = config
        self.input_placeholder = tf.placeholder(dtype=self.config.dtype, shape=[None, None, None])  # (b, h, w, 1)
    ...
    def build(self):
        ...
        atlas_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.labels_placeholder, logits=atlas_logits)
        atlas_train_op = tf.train.AdamOptimizer(config.learning_rate).minimize(atlas_loss)

        return ([(atlas_loss, atlas_train_op)], [atlas_predictions])
        ...
        while True:
            ...
            for id in ids:
                ...
                for start_slice in range(0, max_slice_index, batch_size):
                    end_slice = max(start_slice + batch_size, max_slice_index)
                    yield {input_placeholder: data[start_slice: end_slice], label_placeholder: labels[start_slice]}
...
if __name__ == '__main__':
    ...
    altas_iterator = create_atlas_slice_iterator(slice_network.input_placeholder, slice_network.labels_placeholder, reader, config)
    for epoch in range(config.epochs):
        for atlas_feed_dict in altas_iterator():
            atlas_loss, _ = sess.run(atlas_train_ops, feed_dict=atlas_feed_dict)
